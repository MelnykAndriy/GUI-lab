      ...
      if (page === 1) {
        ...
        if (existingMessages.length > 0) {
          ...
          const hasNewMessages = messages.some(
            (msg) =>
              compareTimestamps(msg.timestamp, lastExistingMessage.timestamp) >
              0,
          );
  ...
  reducers: {
    setSelectedUser: (state, action: PayloadAction<ChatUser | null>) => {
      ...
      if (
        !newSelectedUser ||
        !currentSelectedUser ||
        newSelectedUser.id !== currentSelectedUser.id
      ) {
        state.messages = [];
        state.page = 1;
        state.hasMore = true;
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchMessages.fulfilled, (state, action) => {
        ...
        if (
          !state.selectedUser ||
          action.payload.userId !== state.selectedUser.id
        ) {
        ...
        if (action.payload.isPolling) {
          ...
          if (newMessages.length > 0) {
            state.messages = [...state.messages, ...newMessages].sort((a, b) =>
...
// Actions
export const { setSelectedUser, clearMessages, incrementPage } =
  chatSlice.actions;
